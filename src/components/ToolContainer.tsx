...
import { AlertCircle, Download, CheckCircle2, ArrowLeft, Loader2, Home, X } from 'lucide-react';
...
export default function ToolContainer() {
    ...
    try {
      if (tool.id === 'pdfWatermark') {
        ...
        pages.forEach((page) => {
          ...
          // Get font metrics to scale text
          const textWidthAtSize1 = font.widthOfTextAtSize(watermarkText, 1);
          // Scale font size to cover about 75% of the diagonal for perfect coverage
          const fontSize = (diagonal * 0.75) / textWidthAtSize1;
          const textWidth = font.widthOfTextAtSize(watermarkText, fontSize);
          const textHeight = font.heightAtSize(fontSize);

          // Precise centering for rotated text in pdf-lib
        ...
        data.forEach((row, rowIndex) => {
          ...
          row.forEach((cell, cellIndex) => {
            ...
            x += 100; // Fixed col width for simplicity
          });
          y -= 20;
        });
  ...
  const handleProcess = async () => {
    ...
    // Some tools are handled client-side
  ...
  return (
    ...
    <div className="space-y-12">
        {(!isTextTool && !isImageTool && !isUtilityTool) ? (
